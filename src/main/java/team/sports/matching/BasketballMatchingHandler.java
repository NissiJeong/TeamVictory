   ...
   protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
      ...
      System.out.println("방수입니다:"+rooms.size());
       ...
       System.out.println(session.getId()+"로부터 받은 메시지:"+message.getPayload()); //여기부터문제야
        ...
        area = message.getPayload().split("=")[3];
        ...
        System.out.println(connection.get("area"));
        ...
        */
        ...
        for(int i = 0 ; i < userList.size(); i++) {
          ...
          for(int i = 0 ; i < chatId.size(); i++) {
             System.out.println("쿼리로 뽑아낸 아이디 : "+chatId.get(i).getId());
             for(WebSocketSession client : clients.keySet()) {
                if(clients.get(client).equals(chatId.get(i).getId()) && !userId.equals(clients.get(client))){
