...
public class ChartController {
   ...
   @RequestMapping(value="/Team/Matching/getPitcherChart.do", method = RequestMethod.POST) 
   public @ResponseBody List<Map> makePitcherChart(@RequestParam Map map, Authentication auth) {
      ...
      int homeRunRate = CommonUtils.scoreByNormDist(allHOMERUN, myHOMERUN, homeRunSigma);
   ...
   @RequestMapping(value="/Team/Matching/getPitcherChartTeam.do", method = RequestMethod.POST) 
   public @ResponseBody List<Map> makePitcherChartTeam(@RequestParam Map map, Authentication auth) {
      ...
      if(getMyPit.get(0)!=null && getAllPit.get(0)!=null && getSigma.get(0)!=null) {
         ...
         int eRARate = CommonUtils.scoreByNormDist(allERA, myERA, eRASigma);
         int homeRunRate = CommonUtils.scoreByNormDist(allHOMERUN, myHOMERUN, homeRunSigma);
         int pitchingRate = CommonUtils.scoreByNormDist(allPitching, myPitching, pitchingSigma);
