...
$(function() { 
   ...
   $('#pitcherDetailbtn').click(function() {
      ...
      //차트 변경
      $.ajax({
         url : "<c:url value='/Team/Matching/getPitcherChart.do'/>",
         type : "post",
         data : {
            USER_ID : $('#auth').val(),
            '_csrf' : '${_csrf.token}'
   ...
   $('#pSelectTeam').change(function() {
      //차트 변경
      $.ajax({
         url : "<c:url value='/Team/Matching/getPitcherChartTeam.do'/>",
         type : "post",
         data : {
            USER_ID : $('#auth').val(),
            TEAMNAME : $(this).val(),
                  ...
                  $(data).each(
                        function() {
                           ...
                           str += '<div class="play-table-part listToChange scrollLocation" >' + '<input type="hidden" class="scrolling" data-recordno="'+ this.RECORDNO +'" />' + '<div class="play-table">' + '<div>'
                                 ...
                                 + '<div>'
                                 + '<dl class="info">' + '<dt>'
                                 +  '<h1>'+this.GAMEDATE +'_ '+ this.TIME +'</h1>'
                                 + '</dt>'
                                 + '<dd><h1>'
                                 + this.STADIUM
                                 + '</h1></dd>'
